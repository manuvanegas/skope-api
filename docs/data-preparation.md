...
The checked-in ingest profile currently selects these four annual products:

| Pipeline variable ID | Public S3 object |
| --- | --- |
| `ppt_annual` | `paleocar_v3/ppt_annual/prediction.tif` |
| `ppt_annual_predscaled` | `paleocar_v3/ppt_annual/prediction_scaled.tif` |
| `gdd_cotton_annual` | `paleocar_v3/gdd_cotton_annual/prediction.tif` |
| `gdd_cotton_annual_predscaled` | `paleocar_v3/gdd_cotton_annual/prediction_scaled.tif` |

Before downloading, resolve the current registry mismatch: staging and
production presently advertise `ppt_water_year`, `ppt_may_sept`, `ppt_annual`,
and `gdd_may_sept`. Decide which source products are intended, then update the
ingest and API registries together. The commands below describe the checked-in
four-product ingest profile and must not be used for deployment until that
...
curl --fail --location --continue-at - \
  --output cog-input/ppt_annual.tif \
  https://skope.s3.us-west-2.amazonaws.com/paleocar_v3/ppt_annual/prediction.tif
curl --fail --location --continue-at - \
  --output cog-input/ppt_annual_predscaled.tif \
  https://skope.s3.us-west-2.amazonaws.com/paleocar_v3/ppt_annual/prediction_scaled.tif
curl --fail --location --continue-at - \
  --output cog-input/gdd_cotton_annual.tif \
  https://skope.s3.us-west-2.amazonaws.com/paleocar_v3/gdd_cotton_annual/prediction.tif
curl --fail --location --continue-at - \
  --output cog-input/gdd_cotton_annual_predscaled.tif \
  https://skope.s3.us-west-2.amazonaws.com/paleocar_v3/gdd_cotton_annual/prediction_scaled.tif
```
